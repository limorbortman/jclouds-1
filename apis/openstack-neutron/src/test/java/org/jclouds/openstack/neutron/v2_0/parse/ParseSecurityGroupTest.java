...
public class ParseSecurityGroupTest extends BaseItemParserTest<SecurityGroup> {
   ...
   public SecurityGroup expected() {
      return SecurityGroup.builder()
            .securityGroupRules(
                  ImmutableSet.of(
                        SecurityGroupRule.builder().id("3c0e45ff-adaf-4124-b083-bf390e5482ff").tenantId("1234567890").protocol(SecurityGroupRule.Protocol.TCP).etherType(SecurityGroupRule.EtherType.IPv4).build()
                  )
            )
            .description("jclouds-wibble")
            .name("jclouds-wibble")
            .tenantId("1234567890")
