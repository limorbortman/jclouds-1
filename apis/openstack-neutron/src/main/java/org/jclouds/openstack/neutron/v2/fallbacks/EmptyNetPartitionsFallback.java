...
public class EmptyNetPartitionsFallback implements Fallback<NetPartitions> {

   public ListenableFuture<NetPartitions> create(Throwable t) throws Exception {
      return immediateFuture(createOrPropagate(t));
   }

   @Override
   public NetPartitions createOrPropagate(Throwable t) throws Exception {
      if ((getFirstThrowableOfType(checkNotNull(t, "throwable"), ResourceNotFoundException.class) != null)
            || contains404(t)) {
         return NetPartitions.EMPTY;
      }
      throw propagate(t);
   }
}
